...
def sweets_demo(candies, max_handful):
    ...
    print(
        f'Для победы 1 игроку рекомендуется взять {sweets % (portion + 1)} шт.')
...
def sweets_two(candies, max_handful):
    ...
    while sweets > 0:
        pl_cort = player_step(
            sweets, pl_1, 'Ваш ход. Берите конфеты. Сколько?', 'взял от оставшихся конфет')
        ...
        pl_cort = player_step(
            sweets, pl_2, 'Вы следующий. Берите конфеты', 'взял со стола')
        sweets, score = pl_cort
        if sweets == 0:
            winner = pl_2
            break
...
def sweets_bot(candies, max_handful):
    ...
    portion = max_handful
    print(
        f'Мы договорились брать не более {portion} за раз. Уступаю Вам первый ход')
    print(
        f'И даже подскажу Вам взять {sweets % (portion + 1)} шт. конфет для победы')
    while True:
        score = int(input(f'{pl_1}, сколько конфет берёте?  '))
        if score > min(portion, sweets):
            print(
                f'Упссс. Договаривались не более {portion}! Верните лишние ))')
        else:
            break
    sweets -= score
    ...
    while sweets > 0:
        ...
        score = sweets % (portion + 1)
        print(
            f"Угу.. осталось {sweets}. Мы с фиксиками попробуем {score} шт. Они любят сладкое ))")
        sweets -= score
        if sweets == 0:
            winner = pl_2
            break
        print(
            f"Осталось {sweets} шт. Берите быстрей, а то фиксики чаю с конфетами очень хотят!")
        while True:
            score = int(
                input(f'{pl_1}, я бы взял {sweets % (portion + 1)}, а Вы сколько берёте? '))
            if score > min(portion, sweets):
                print(
                    f'Упссс. Договаривались не более {portion}! Верните лишние ))')
            else:
        ...
        if sweets == 0:
            ...
            break
        print(
            f"В сладком остатке {sweets} шт. Неужели моя видеокарта зря кипятилась!")
